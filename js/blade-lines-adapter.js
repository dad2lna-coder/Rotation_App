...
(function (global) {
  ...
  function parseTime(value) {
    ...
    return { startMin: startMin, endMin: endInternal, start: fmt(startMin), end: fmt(endMin), shift: shift, raw: raw };
  }
  ...
  function adapt(lines, day, options) {
    ...
    lines.forEach(function (line, index) {
      ...
      if (!rawStr || OFF_RE.test(rawStr)) {
        skipped.push({ index: index, line: lineDisplayName(line, index), key: lineKey(line, index), raw: rawStr, reason: !rawStr ? "empty" : "non-working" });
        return;
      }
      var locOpt = options.location;
      if (typeof options.locationFor === "function") locOpt = options.locationFor(line, di, index);
      var placeOpt = null;
      if (typeof options.placementFor === "function") placeOpt = options.placementFor(line, di, index);
      ...
      if (!row) {
        skipped.push({ index: index, line: lineDisplayName(line, index), key: lineKey(line, index), raw: rawStr, reason: "unparseable-time" });
        return;
      }
      rows.push(row);
    });
